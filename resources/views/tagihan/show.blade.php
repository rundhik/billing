				...
				<table class="table invoice-items">
					...
					<tbody> 
                        @for ($i = 0; $i < $count; $i++)
                            @if ($i == 0)
                                <tr>
									<td>Pemakaian {{ $t->tarif[$i][0] }} - {{ $t->tarif[$i][1] }} {{ $tar->find($t->penggunaan->layanan_id)->satuan }}</td>
                                    <td class="text-center">{{ number_format($t->tarif[$i][2],0,'.',',') }}</td>
                                    <td class="text-center">@if ($u > ($t->tarif[$i][1]-$t->tarif[$i][0]) ) {{ $t->tarif[$i][1]-$t->tarif[$i][0] }} @else {{ $u }} @endif @php $x = $u; $u = $u-($t->tarif[$i][1]-$t->tarif[$i][0]); @endphp</td>
                            ...
                            @elseif ( $i > 0 && $i < ($count-1))
                                <tr>
                                    <td>Pemakaian > {{ $t->tarif[$i][0] }} - {{ $t->tarif[$i][1] }} {{ $tar->find($t->penggunaan->layanan_id)->satuan }}</td>
                                    <td class="text-center">{{ number_format($t->tarif[$i][2],0,'.',',') }}</td>
                                    <td class="text-center">@if ($u > ($t->tarif[$i][1]-$t->tarif[$i][0]) ) {{ $t->tarif[$i][1]-$t->tarif[$i][0] }} @else {{ $u }} @endif @php $u = $u-($t->tarif[$i][1]-$t->tarif[$i][0]); @endphp</td>
                                    <td class="text-center">{{ number_format($t->tagihan[$i],0,'.',',') }} </td>
                                </tr>
                            @else
                                <tr>
                                    <td>Pemakaian > {{ $t->tarif[$i][0] }} {{ $tar->find($t->penggunaan->layanan_id)->satuan }}</td>
                                    <td class="text-center">{{ number_format($t->tarif[$i][2],0,'.',',') }}</td>
